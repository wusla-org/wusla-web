...
useHead({ meta: [{ name: "theme-color", content: "#FBFBEA" }], bodyAttrs: { class: "w-body" } });
...
<template>
  <main class="start-page">
    <header class="start-header">
      <NuxtLink to="/" class="start-wordmark" aria-label="WUSLA home" translate="no">WUSLA</NuxtLink>
...
.start-page{--w-hair-strong:rgba(20,20,15,.22);min-height:100dvh;background:var(--w-cream);color:var(--w-ink);font-family:var(--w-sans)}
.start-header{position:sticky;z-index:20;top:0;display:flex;align-items:center;gap:24px;justify-content:space-between;padding:18px 40px;height:80px;border-bottom:1px solid var(--w-hair);background:var(--w-cream)}
.start-wordmark{font-size:1.0625rem;font-weight:600;letter-spacing:-.008em;text-decoration:none;color:var(--w-ink);margin-right:auto}.return-link{display:inline-flex;align-items:center;gap:8px;min-height:44px;font-size:.875rem;color:var(--w-ink-soft)}.return-link:hover{color:var(--w-ink)}
.intake-shell{display:grid;grid-template-columns:minmax(0,42%) minmax(0,58%);min-height:calc(100dvh - 80px)}
.intake-intro{position:sticky;top:80px;align-self:start;min-height:calc(100dvh - 80px);display:flex;flex-direction:column;padding:52px clamp(24px,4vw,66px) 32px;background:var(--w-cream-2)}
.intro-label,.question-context{font:500 .8125rem/1.5 var(--w-sans);color:var(--w-emerald)}
.intro-copy h1{font-family:var(--w-serif);font-weight:400;font-size:clamp(2.65rem,4.2vw,4.5rem);letter-spacing:-.022em;line-height:1.04;margin-block:22px;max-width:9ch}
.intro-note{font-size:1rem;line-height:1.75;color:var(--w-ink-soft);max-width:390px}
.step-list{list-style:none;padding:0;margin:38px 0 30px}.step-list li{border-top:1px solid var(--w-hair)}.step-list button{display:grid;grid-template-columns:26px 1fr 18px;gap:14px;align-items:center;width:100%;text-align:left;min-height:44px;font-size:.875rem;color:var(--w-ink-soft)}
.step-list .active button,.step-list .complete button{color:var(--w-ink)}.step-list .active button{font-weight:700}.step-number{font:400 .75rem var(--w-sans);color:var(--w-emerald)}.privacy-note{font-size:.8125rem;color:var(--w-ink-soft);margin-top:auto}
.letter-surface{position:relative;display:flex;flex-direction:column;min-width:0;padding:36px clamp(24px,4.5vw,76px) 30px}
.letter-header{display:flex;justify-content:space-between;gap:20px;padding-bottom:20px;border-bottom:1px solid var(--w-hair);font:400 .8125rem var(--w-sans);color:var(--w-ink-soft)}
...
.question-context{margin-bottom:16px}.question-panel h2,.success-panel h2{font-family:var(--w-serif);font-weight:400;max-width:16ch;font-size:clamp(2rem,3.2vw,3.4rem);line-height:1.1;letter-spacing:-.02em}.question-panel h2:focus{outline:none}
.question-help{margin-top:20px;color:var(--w-ink-soft);line-height:1.7;font-size:1rem}
.large-text-field{display:block;margin-top:28px;border-bottom:1px solid var(--w-hair-strong)}.large-text-field:focus-within{outline:2px solid var(--w-emerald);outline-offset:6px}
.large-text-field textarea{width:100%;display:block;background:transparent;border:0;resize:vertical;min-height:155px;font-size:1.125rem;color:var(--w-ink);line-height:1.7;outline:none}
textarea::placeholder,input::placeholder{color:var(--w-ink-soft)}
.field-meta{display:flex;justify-content:space-between;gap:10px;padding-block:12px;font:400 .75rem var(--w-sans);color:var(--w-ink-soft)}
.choice-list{margin-top:28px;border-top:1px solid var(--w-hair)}.choice-list label{position:relative;display:grid;grid-template-columns:24px 1fr;gap:15px;align-items:start;padding:17px 10px;border-bottom:1px solid var(--w-hair);cursor:pointer;border-radius:10px;transition:background 160ms}
.choice-list label:hover,.choice-list label.selected{background:var(--w-cream-2)}.choice-list label:focus-within{outline:2px solid var(--w-emerald);outline-offset:2px}
.choice-list input{position:absolute;opacity:0;width:1px;height:1px}.choice-mark{display:grid;place-items:center;width:22px;height:22px;border:1px solid var(--w-hair-strong);border-radius:50%;color:transparent;margin-top:2px}.selected .choice-mark{background:var(--w-emerald);border-color:var(--w-emerald);color:var(--w-bone)}
.choice-copy{display:flex;flex-direction:column;gap:6px}.choice-copy strong{font-size:1rem;font-weight:600}.choice-copy small{font-size:.875rem;line-height:1.6;color:var(--w-ink-soft)}
.line-field{display:flex;flex-direction:column;gap:10px;min-width:0}.line-field>span{font-size:.875rem;font-weight:600}.line-field small{display:block;font-size:.75rem;font-weight:400;color:var(--w-ink-soft);line-height:1.5}
.line-field input{background:var(--w-cream-2);border:1px solid var(--w-hair-strong);border-radius:10px;padding:13px;width:100%;min-height:48px;font-size:1rem;color:var(--w-ink)}
.budget-field{margin-top:25px}.contact-grid{display:grid;grid-template-columns:1fr 1fr;gap:24px 20px;margin-top:30px}
.letter-footer{display:flex;align-items:center;justify-content:space-between;gap:20px;padding-top:24px;border-top:1px solid var(--w-hair)}
.back-button,.next-button{display:inline-flex;align-items:center;justify-content:center;gap:10px;min-height:48px;font-size:.9375rem;font-weight:500;font-family:var(--w-sans);cursor:pointer;transition:background 200ms var(--w-ease),border-color 200ms var(--w-ease)}.back-button{color:var(--w-ink-soft);padding:12px 0;background:none;border:0}.back-button:hover{color:var(--w-ink)}
.next-button{background:var(--w-emerald);color:var(--w-bone);border:1px solid var(--w-emerald);border-radius:999px;padding:0 22px;white-space:nowrap}.next-button:hover:not(:disabled){background:var(--w-emerald-lit);border-color:var(--w-emerald-lit)}.next-button:disabled{opacity:.45}
.direct-email{font-size:.8125rem;color:var(--w-ink-soft);max-width:230px}.direct-email a{display:block;text-decoration:underline;overflow-wrap:anywhere}
.progress-rule{height:3px;background:var(--w-hair);margin-top:26px}.progress-rule span{height:100%;display:block;background:var(--w-emerald);transition:width 250ms}
.error-message{margin-top:20px;font-size:1rem;color:var(--w-ink);border-left:3px solid var(--w-emerald);padding-left:15px}.error-message a{text-decoration:underline}
.success-panel{padding:60px 0;display:flex;flex:1;flex-direction:column;justify-content:center}.success-mark{display:grid;place-items:center;background:var(--w-emerald);color:var(--w-bone);width:54px;height:54px;border-radius:50%;margin-bottom:25px}.success-panel>p:not(.question-context){margin-top:24px;font-size:1.0625rem;line-height:1.75;color:var(--w-ink-soft)}
.success-actions{display:flex;gap:25px;flex-wrap:wrap;margin-top:30px}.success-actions a{display:inline-flex;align-items:center;gap:8px;min-height:44px;border-bottom:1px solid var(--w-hair-strong);font-size:.9375rem}
.brief-review{margin-top:30px}.brief-review>div{padding:18px 0;border-top:1px solid var(--w-hair)}.brief-review dt{display:flex;justify-content:space-between;align-items:center;gap:20px;font-size:.875rem;font-weight:600}.brief-review dt button{font-size:.8125rem;color:var(--w-emerald);text-decoration:underline;min-height:44px;min-width:44px}.brief-review dd{margin:5px 0 0;font-size:1rem;line-height:1.7;color:var(--w-ink-soft);overflow-wrap:anywhere}.review-message{white-space:pre-wrap}
